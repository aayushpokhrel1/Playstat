...
- **The search is now a single unbounded pass bounded by an exact heap-aware prune, provably identical to a global brute force** on every slate — no assumption about odds/probability correlation. It replaced a progressive-widening early-stop that was exact only under uniform-vig book lines. The investigation that motivated the rewrite: the early-stop had **0 optimum-misses on the real live slate** at targets 1.3–2.5x and **0/894 on a realistic tight-vig ensemble**; it only missed (~0.5%, worst ~1.6pp) under an unrealistic wide vig spread. So this was a provable-exactness hardening, not a fix to an observed production error.
  ...
  - **Node counts depend heavily on slate structure — the two axes differ, and synthetic vs. real slates differ sharply.** On a *synthetic* uniform-odds ~2,400-leg / 19-game slate (`top_n=5`) the payout axis finishes exhaustively (~287k nodes) and `min_prob=0.75` at ~3.25M. But **real slates are favourite-heavy** (most legs ~1.0–1.1x, since the builder takes market-prob≥0.55 favourites), so reaching a 1.4x/2.0x *floor* forces the search deep and the payout-axis prune is weak: on the **live 920-leg / 7-game slate the payout axis truncates at the 5M budget (~24–27 s per query)** while `min_prob=0.75` finishes exhaustively (~780k nodes, ~2 s). **Truncation does not compromise the result** — the size-2 pass completes before the budget is hit, and for a payout floor fewer legs is always strictly safer, so the top (2-leg) construction is exact regardless; only hypothetical deeper 3–4-leg constructions (which are never safer) go unsearched. The `truncated` flag is shown honestly. **Known cost of the provable-exactness rewrite:** removing the widening early-stop made real-slate payout queries slower (seconds → ~25 s) for a guarantee that only bites under wide per-leg vig variance not seen on real consensus book lines (see the 0-miss investigation above). The dashboard is architected around this — it server-renders the precomputed nightly rows instantly and only the explicit "Build" runs a live search — but tightening the payout-axis prune for favourite-heavy slates is open follow-up work.
