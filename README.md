...
- **The search is now a single unbounded pass bounded by an exact heap-aware prune, provably identical to a global brute force** on every slate — no assumption about odds/probability correlation. It replaced a progressive-widening early-stop that was exact only under uniform-vig book lines. The investigation that motivated the rewrite: the early-stop had **0 optimum-misses on the real live slate** at targets 1.3–2.5x and **0/894 on a realistic tight-vig ensemble**; it only missed (~0.5%, worst ~1.6pp) under an unrealistic wide vig spread. So this was a provable-exactness hardening, not a fix to an observed production error.
  - Two exact suffix-maximum look-ahead prunes bound the search (the odds dual `best_from` and the probability dual `best_prob_from`), plus a heap-aware top-N prune on each axis. All are verified exact by the pure global brute-force oracle (`tests/test_builder_search_exactness.py`), including its independent-draw adversarial cases. `MAX_NODES` (5M) remains a hard bound and the `truncated` flag is surfaced when hit.
  - **Node counts depend heavily on slate structure — the two axes differ, and synthetic vs. real slates differ sharply.** On a *synthetic* uniform-odds ~2,400-leg / 19-game slate (`top_n=5`) the payout axis finishes exhaustively (~287k nodes) and `min_prob=0.75` at ~3.25M. But **real slates are favourite-heavy** (most legs ~1.0–1.1x, since the builder takes market-prob≥0.55 favourites), which forces the search deep, and the effectiveness of the heap-aware prune also depends on `top_n` (a larger heap takes longer to fill, so it prunes later). At the API/dashboard default **`top_n=10`**, the live 920-leg / 7-game slate **truncates on both axes**: `target_payout=1.4`/`2.0` hit the 5M budget in ~24–27 s, and `min_prob=0.75` also hits it in ~7 s. (At `top_n=5` the min_prob axis finishes exhaustively in ~2 s and the payout axis still truncates — so exact figures are `top_n`- and slate-dependent; do not treat any single measurement as the general case.) **Truncation does not compromise the result** — the size-2 pass completes before the budget is hit, and for a payout floor fewer legs is always strictly safer, so the top (2-leg) construction is exact regardless; only hypothetical deeper 3–4-leg constructions (which are never safer) go unsearched. The `truncated` flag is shown honestly. **Known cost of the provable-exactness rewrite:** removing the widening early-stop made real-slate payout queries slower (seconds → ~25 s) for a guarantee that only bites under wide per-leg vig variance not seen on real consensus book lines (see the 0-miss investigation above). The dashboard is architected around this — it server-renders the precomputed nightly rows instantly and only the explicit "Build" runs a live search — but tightening the payout-axis prune for favourite-heavy slates is open follow-up work.
