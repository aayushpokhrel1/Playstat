...
- **Daily-job observability — READY TO APPLY, not yet applied**: the user created both free accounts 2026-07-16 and ntfy delivery to their iPhone is confirmed working; the values live in the **gitignored `.env`** as `HEALTHCHECK_PING_URL` and `NTFY_TOPIC` (capability tokens — never commit them; that's why they're not written here). The remaining step is one `com.playstat.mlb.plist` edit that does **two** things at once: insert `python -m modeling.settle` right after `modeling.clv` (the deferred paper-trading ledger step, §14.1) **and** wrap the chain as `{ <chain> && curl -fsS -m 10 --retry 3 $HEALTHCHECK_PING_URL ; } || curl -fsS -m 10 -d "Playstat MLB daily chain failed" https://ntfy.sh/$NTFY_TOPIC` — the ping is the last `&&` link so it fires only on full success, and any earlier failure short-circuits it and triggers the ntfy push. Use absolute `/usr/bin/curl` (launchd's PATH is minimal). Then `launchctl unload && load` the plist, and smoke-test both paths before trusting it. Held only for the 2026-07-17 9:30am review freeze. Original research below: in `com.playstat.mlb.plist`'s `bash -c` chain string, append `&& curl -fsS -m 10 --retry 3 https://hc-ping.com/<uuid>` as the final link (only reached on full success), and wrap the whole chain in `{ ...; } || curl -d "MLB daily chain failed" ntfy.sh/<topic>` for same-morning failure pushes. Optionally also a "last successful run" indicator on the dashboard from a `job_runs` row.
