...
**Current data state**: `model_predictions` and `backtest_runs` have real historical data (the model has actually been trained and backtested). `prop_lines` has **live MLB lines** as of 2026-07-13 (~1,000 rows per pull across 13 stat types, via `ingestion/odds_ingest.py --sport mlb`) — the first real odds data in the project. `edges` and `parlay_recommendations` are still empty: edges need model predictions for the games the lines are on, and MLB modeling isn't built yet (NBA predictions exist but NBA has no lines until ~October). The moment either sport has both lines *and* predictions, `/edges` lights up.
...
- **Daily MLB job — scheduled**: `com.playstat.mlb` (launchd, 8:30am, `~/Library/LaunchAgents/`, logs to `logs/mlb.log`) chains the full loop: box scores → linescores → features (+2d upcoming) → predictions → odds snapshot → first-inning model → edges → parlay (2x target, ≤3 legs).
...
- **Model**: `modeling/first_inning.py` — features are each team's rolling 15-game first-inning runs *scored* and *allowed*; P(under 1.5) is a small XGBoost binary classifier (a Poisson conversion from predicted mean ran ~7 points low on holdout — first-inning runs are overdispersed vs Poisson). Holdout honesty: Brier 0.2005 vs 0.1972 for always-predicting-the-base-rate, i.e. **team form alone adds ~no signal beyond the prior; the probabilities are calibrated, not sharp**. The v2 that would actually add signal is the probable starting pitcher (StatsAPI `hydrate=probablePitcher`; starter form is derivable from our pitcher stats — the game's starter is the team's max-`outs_recorded` pitcher).
...
- **Line-movement / closing-line-value (CLV) tracking**: `prop_lines` already snapshots with `pulled_at`; compare the line at bet time (or edge-detection time) against the closing line. CLV is the best early predictor of whether edges are real — it gives an answer weeks before win/loss records are statistically meaningful.
