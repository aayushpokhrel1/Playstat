...
- **The 2.0x search truncates.** It hits its 5M-node budget on a full slate and reports `WARNING: search hit its node budget — results are partial, not exhaustive`. 2-leg constructions *are* searched exhaustively (~86k nodes) and 2 legs is provably the safest route to any payout (least vig), so the top results are almost certainly optimal — but this is not proven, and the warning is deliberately surfaced rather than hidden.
- **A target payout returns the bottom of its tolerance band.** With `--target-payout 2.0 --tolerance 0.15` the band is [1.7, 2.3] and ranking by joint probability always prefers the least-risky end, so you ask for 2.0x and get ~1.72x. This is *correct* given the agreed semantics ("safest construction near the target"), and the real payout is always displayed — but if you want closer to a true 2.0x, tighten `--tolerance`. **Open product question for the user.**
- **Team legs are thin** (3 of 673). `game_lines` holds 107 NRFI + 45 F5 lines but was last pulled 2026-07-20, so few map to unfinished games. The player+team mixing path is structurally exercised and correct; it is data freshness, not a code gap.
- **Not yet in the daily chain.** `scripts/daily_chain.sh` still runs the old OOM-prone `optimizer.parlay` step. Swapping it is the remaining architect task and is what actually stops the nightly failure alert.
