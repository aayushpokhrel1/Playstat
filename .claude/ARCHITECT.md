...
- **Worktrees lack gitignored files.** Tell agents to copy `.env` (and `web/.env.local` if needed) from the main checkout, and to `npm install` in `web/` (node_modules aren't checked out). Main checkout venv: `/Users/aayushpokhrel/dev/playstat/.venv`.
